...
// GetImageByNameOrUUID returns an image with the UUID. If no UUID is provided, returns the unique image with the name.
// Returns an error if no image has the UUID, if no image has the name, or more than one image has the name.
func GetImageByNameOrUUID(ctx context.Context, client *prismclientv3.Client, imageName, imageUUID *string) (*prismclientv3.ImageIntentResponse, error) {
	...
	if imageUUID != nil {
		resp, err := client.V3.GetImage(ctx, *imageUUID)
		if err != nil {
			if strings.Contains(fmt.Sprint(err), "ENTITY_NOT_FOUND") {
	...
	} else if imageName != nil { // else search by name
...
func ImageMarkedForDeletion(image *prismclientv3.ImageIntentResponse) bool {
	...
	return state == "DELETE_PENDING" || state == "DELETE_IN_PROGRESS", nil
}
